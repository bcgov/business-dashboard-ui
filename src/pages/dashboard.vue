...
const alerts = computed((): Array<Partial<AlertI>> => {
  ...
  const notInGoodStanding = currentBusiness.value?.goodStanding === false || Boolean(notInGoodStandingWarning)
  ...
  if (notInGoodStanding && (!liquidationWarning || hasOverdueLiquidationReport)) {
  ...
  if (allWarnings.some(item => item.warningType === WarningTypesE.FUTURE_EFFECTIVE_AMALGAMATION)) {
    ...
    alertList.push({
      ...
      date: amalDate,
      options: {
        amalgamatingBusinesses: warning?.data?.amalgamatingBusinesses || [],
        amalgamatingIntoName: warning?.data?.resultingBusinessName,
        currentBusinessIdentifier: currentBusiness.value?.identifier,
        date: amalDate,
        filingId: warning?.data?.filingId,
        resultingBusinessIdentifier: warning?.data?.resultingBusinessIdentifier
      }
    })
  }

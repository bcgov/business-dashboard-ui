...
const props = defineProps({
  ...
  uploadDocumentToDrs: { type: Function, required: true }
})
...
const uploadFile = async (file: File): Promise<string | null> => {
  try {
    const response = await props.uploadDocumentToDrs(
      file,
      DOCUMENT_TYPES.CourtOrder.class,
      DOCUMENT_TYPES.CourtOrder.type,
      currentBusinessIdentifier.value
    )

    if (response.status.value === 'success') {
      return response.data.value.documentServiceId
    } else {
